...
int get_temp(struct get_temp_t &s){
    ...
    PT_SPAWN(&s.pt, &s.child, mcp3424_measure(&s.child,s.addr,4,s.raw[1]));

    /* read from ch2 - top    */
    PT_SPAWN(&s.pt, &s.child, mcp3424_measure(&s.child,s.addr,2,s.raw[2]));
...
double rtd_calc(int32_t raw){
  ...
  double ratio = volts * (10.09 / 5.0); // 5V supply, voltage divider using 909-ohm resistor (and 100-ohm RTD)
